...
Importing the included Node-RED flow

- The repository contains a ready-to-import flow: `NodeRed Flow/digital_switch_relais_control.json`.
- To import: open Node-RED → menu → Import → File/Clipboard and select/paste the JSON.
- After import, configure the Signalk nodes: replace the `source` placeholder `<YOUR_SIGNALK_WS>` with your Signal K WebSocket or point the nodes to your Signal K server, and adjust the `path` if needed.
- See `NodeRed Flow/README.md` for details.
